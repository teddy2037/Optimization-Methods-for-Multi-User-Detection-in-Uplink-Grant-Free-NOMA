function [x,iter] = fixed_f_admm_fun(y,H,T,Q,K,N,M,SNR,lam,max_iter,tolerance,eta)
%%%%%%%%%%%%%%%%%%% convert to real values %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if nargin == 11
    eta = 0.35;  % or whatever
end
ytrans = y;
Htrans = H;
x = zeros(K,1);
...
while (norm(r,2) > tolerance || norm (s,2) > tolerance) && max_iter >= iter
    ...
    for t = 1:T
        %%%%%%%%%%%%%%% step-4 %%%%%%%%%%%%
        x_current(:,t) = V(:,:,t)*inv(L(:,:,t)+(row(t) + alpha_prev(t,1))*eye(2*K))*V(:,:,t)'*(eff(:,:,t) + alpha_prev(t,1)*x_prev_hat(:,t) + row(t)*(z_mod_prev(:,t)-u_mod_prev(:,t)));
